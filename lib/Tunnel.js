...
var Tunnel = function(opt) {
    ...
    self._opt.reconnect_count = self._opt.reconnect_count || 0;
};
...
Tunnel.prototype._establish = function(info) {
    ...
    tunnels.once('open', function() {
        ...
        // TODO: reset reconnect_count
    });

    // re-emit socket error
    tunnels.on('error', function(err) {
        var message = err.message;
        var reconnect = self._opt.reconnect;
        if(reconnect) {
          self.reconnect(/* function(count, client) {} */);
...
Tunnel.prototype.reconnect = function(cb) {
    var self = this;
    typeof cb != "function" && (cb = function(count, client) {});
    opt = self._opt || {};
    var count = opt.reconnect_count++;
    var max_conn = self.tunnel_cluster._opt.max_conn;
    if ((count + 1) % max_conn != 0) {
      return cb(count, null);
    }
    debug("Reconnecting... %s", count);
    var client = self; //new Tunnel(opt);
